    ...
    fn hint(&self) -> Option<String> {
        match self {
            Self::LeaseHeld(detail) => Some(if detail.path_conflict {
                "another workspace already claims this directory; release it, or run \
                 `libra worktree doctor` if its owner is gone"
                    .to_string()
            } else {
                "wait for the holder to release the lease, or run `libra worktree doctor` to \
                 reclaim it once the lease has expired and its owner is confirmed dead"
                    .to_string()
            }),
            Self::LeaseLost { .. } => Some(
        ...
        match RepoIdentity::resolve(conn).await {
            ...
            Ok(current) => WorkspaceError::Corrupt(format!(
                "workspaces registered under a previous repository identity are still live or \
                 ...
                 `libra worktree doctor` to adopt or release them"
            )),
            Err(error) => error,
        }
    }
