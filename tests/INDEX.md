...
| `compat_status_wave0_register` | 1 | plan-20260714 B.9 registration gate: `STATUS_WAVE0_TESTS` manifest ↔ `command::status_wave0::*` bidirectional set equality + strict ordering, so the wave-0 status module cannot be silently dropped from CI; `#[cfg(unix)]` cases ride the `STATUS_WAVE0_TESTS_UNIX_ONLY` subset and are excluded off-Unix | `tests/command/status_wave0_test.rs`, `tests/compat/status_wave0_manifest.rs` |
