...
/// A registry lane is `global` or `lane:<key>` with a non-empty snake_case key.
fn lane_is_valid(lane: &str) -> bool {
    if lane == "global" {
        return true;
    }
    let Some(key) = lane.strip_prefix("lane:") else {
        return false;
    };
...
/// and ignores `#[serial]` inside single- and multi-line string literals.
#[test]
fn classifier_ignores_string_literals_and_reads_same_line_attributes() {
    ...
    assert_eq!(
        stdout, "real_serial\tnone\nsame_line_multi_attr\tnone\n",
        "string literals must not produce rows and same-line attributes must be read"
    );
}
