...
| `compat_status_wave0_register.rs` (+ `status_wave0_manifest.rs`) | plan-20260714 R0-0 | `STATUS_WAVE0_TESTS` canonical manifest ↔ direct `#[test]`/`#[tokio::test]` declarations in the registered `tests/command/status_wave0_test.rs` module: bidirectional set equality without recursively spawning Cargo (which would deadlock on Cargo's target lock); strict alphabetical ordering; non-empty guard; `#[cfg(unix)]` cases are declared in the `STATUS_WAVE0_TESTS_UNIX_ONLY` subset |
