...
SSE v2 projects a committed receipt as the dedicated
`kind: "intent_revision_consumed"` with payload `{ "consumption": ... }`; it
omits generic resolution fields and the raw note. A crash before durable effect
