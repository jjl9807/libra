...
**Linked worktrees**: `libra code` (every mode) still refuses to launch from a linked worktree until W4-08 enablement. Security-sensitive files (`sandbox.toml`, `hooks.json`, `config.toml` `[approval]`/`[mcp]`, `rules/`, `contexts/`) already resolve through the W4-06 RequestScope resolver: repository defaults stay visible, linked overlays may only tighten (they cannot drop a repository `PreToolUse` Block or loosen sandbox/approval), and unreadable or malformed security config fails closed with a source-layer diagnostic (no file contents). Agent/automation extension loaders remain on the pre-W4-12 path. Run from the main worktree; the refusal names this remedy. Automation VCS dispatch stays disabled (with a warning) in linked worktrees and `libra automation` fails closed there — see [automation.md](automation.md).
