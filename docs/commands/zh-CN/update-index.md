...
`update-index` 按顺序应用：所有 `--cacheinfo` 条目，然后是位置路径（带 `--remove` 则删除，否则从工作树（重新）暂存），最后保存 index。`--add` 与 `--remove` 是两项**许可**而非互斥选项，可以同时给出：此时逐路径按其**在工作树中是否存在**分流——仍存在的被暂存，已消失的从 index 移除。
