...
| Code Web-only / Agent runtime | W4 bake window: default Web Code UI is live (W4-01); thread/version graph is in Web Code UI (W4-04); `--web`/`--web-only` are deprecated no-op aliases; hidden `LIBRA_CODE_LEGACY_TUI=1` is emergency rollback only. Bare `--provider codex --resume` still uses the legacy TUI resume driver. Physical TUI/`--web*`/`code-control`/interactive graph removal is W5-01 family (W5-09). `WorkflowPattern` and per-workflow budget remain open. | `CodeArgs` still accepts `--web/--web-only` and `--stdio`; default `execute()` is Web, not `execute_tui()`, except `LIBRA_CODE_LEGACY_TUI=1` and bare `--provider codex --resume`. `BudgetScope` is still Session/Agent/Goal; no `WorkflowPattern` layer. | W5-01 family deletes deprecated public surfaces after the 3-patch bake; do not claim physical TUI/graph/PTY removal before W5-09. Remaining AG work is in `docs/development/internal/code-agent-runtime.md`. |
