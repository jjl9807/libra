...
| Code Web-only / Agent runtime | W4 bake window closed: default Web Code UI is live (W4-01); thread/version graph is in Web Code UI (W4-04); W5-07 removed the deprecated `--web`/`--web-only` aliases and the hidden `LIBRA_CODE_LEGACY_TUI=1` rollback env (old flags now fail with a usage error plus migration hint). Bare `--provider codex --resume` still uses the legacy TUI resume driver until W5-06. Physical TUI startup path/interactive graph removal remains in the W5-01 family (W5-06/W5-08/W5-09). `WorkflowPattern` and per-workflow budget remain open. | `CodeArgs` no longer exposes `--web/--web-only` (W5-07); default `execute()` is Web, not `execute_tui()`, except bare `--provider codex --resume`. `BudgetScope` is still Session/Agent/Goal; no `WorkflowPattern` layer. | W5-01 family deletes deprecated public surfaces after the 3-patch bake (W5-01 code-control shim, W5-07 web aliases/rollback env done); do not claim physical TUI/graph/PTY removal before W5-09. Remaining AG work is in `docs/development/internal/code-agent-runtime.md`. |
