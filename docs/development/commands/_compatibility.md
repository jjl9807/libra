...
| Code Web-only / Agent runtime | W4 bake window closed: default Web Code UI is live (W4-01); thread/version graph is in Web Code UI (W4-04); W5-07 removed the deprecated `--web`/`--web-only` aliases and the hidden `LIBRA_CODE_LEGACY_TUI=1` rollback env (old flags now fail with a usage error plus migration hint). W5-06 removed the Code TUI startup path and the bare `--provider codex --resume` legacy TUI resume driver (now a usage error plus migration hint; every non-stdio `libra code` launches the Web Code UI). W5-08 removed the interactive graph TUI entry. What remains in the W5-01 family is the W5-09 release point; `src/internal/tui` module removal (W5-03) and ratatui/crossterm dependency removal (W5-10) are internal cleanups. `WorkflowPattern` and per-workflow budget remain open. | `CodeArgs` no longer exposes `--web/--web-only` (W5-07); default `execute()` is Web-only (`code_uses_web_launch = !stdio`), and bare `--provider codex --resume` is rejected by `validate_mode_args` (W5-06). `BudgetScope` is still Session/Agent/Goal; no `WorkflowPattern` layer. | W5-01 family deletes deprecated public surfaces after the 3-patch bake (W5-01 code-control shim, W5-07 web aliases/rollback env, W5-06 TUI startup path, W5-08 graph entry done); W5-09 is the single release point. Remaining AG work is in `docs/development/internal/code-agent-runtime.md`. |
