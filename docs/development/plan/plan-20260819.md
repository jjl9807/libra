...
**模板版本：** 依据 [`plan-template.md`](plan-template.md) `v2` 编写。模板中的 `GC-*`、`ER-*` 与 `G-*` 全部适用；本文只补充 M2 研发历程记忆（下文简称 M2）的领域约束、任务卡和验收口径。
...
**发布窗口顺序：** 严格按 DAG；同一时刻仅一张卡进入暂存/提交/push 窗口。M2-15 做一次 patch 版本面收口并推送贡献分支；本计划不创建或推送 release tag，release artifact 与 D-02 均为 N/A，由 maintainer 后续 release 流程拥有。D-01 以本次 branch/PR SHA 的实际 CI checks 为准。
...
- **Release write set 默认：** batch child 为 `N/A`；M2-15 继承模板版本面与 release artifact。
...
**C/D coverage from:** `self`（batch child 三门与 D-01；版本面/D-02 由 M2-15）
...
**C/D coverage from:** `self`（D-01 精确合同为 DEP-M2-CI-01；batch child 状态仍按 REL-M2-01 保持 `locally-accepted` 到 M2-15）
...
**C/D coverage from:** `self`（四平台 D-01 精确执行面为 DEP-M2-CI-01；batch child 状态仍按 REL-M2-01 保持 `locally-accepted`）
...
**C/D coverage from:** `self`（D-01 精确执行面为 DEP-M2-CI-01/02；D-02 为 N/A）
