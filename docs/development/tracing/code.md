...
- `--stdio` 是弃用的 MCP-only legacy transport（tools/resources；非 turn control；stderr 弃用警告）。源码在 `validate_mode_args` 中明确拒绝 `--control write` 并提示使用 `libra code --control stdio` 做本地 automation（遗留 `code-control --stdio` 仍可用至 W4-09）。独立 `libra mcp --stdio` 为 DEFER-02（W5 之后），本计划不新增半成品 `libra mcp`。
...
| MCP / code-control | `libra code --stdio` 是弃用的 MCP-only legacy（tools/resources；非 turn control）；canonical automation client 是 `libra code --control stdio`（遗留 `code-control --stdio` 至 W4-09）；`libra mcp --stdio` 为 DEFER-02。 | C6 禁止把 MCP stdio 当 turn control plane；双入口 tool set、shutdown、token/lease gate 都要有测试。 |
