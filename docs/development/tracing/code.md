...
| C6 | `code --stdio`（`execute_stdio`）是 MCP transport；canonical automation client 是 `code --control stdio`（`ControlMode::Stdio` + `run_control_stdio_client` + W4-10 control-info discovery）；遗留 `code-control --stdio` 至 W4-09。 | MCP 与 control client 不可混作 turn control plane；discovery/attach fail-closed（`CONTROL_*` / JSON-RPC `-32000`）。 | W4-02/W4-09/W4-10；breaking command migration。 |
