...
- `--stdio` 是 MCP stdio transport。源码在 `validate_mode_args` 中明确拒绝 `--control write` 并提示使用 `libra code --control stdio` 做本地 automation（遗留 `code-control --stdio` 仍可用至 W4-09）。
...
| MCP / code-control | `libra code --stdio` 是 MCP stdio server；canonical automation client 是 `libra code --control stdio`（遗留 `code-control --stdio` 至 W4-09）。 | C6 禁止把 MCP stdio 当 turn control plane；双入口 tool set、shutdown、token/lease gate 都要有测试。 |
...
| C6 | `code --stdio`（`execute_stdio`）是 MCP transport；canonical automation client 是 `code --control stdio`（`ControlMode::Stdio` + `run_control_stdio_client`）；遗留 `code-control --stdio` 至 W4-09。 | MCP 与 control client 不可混作 turn control plane；discovery/attach 失败契约归 W4-10。 | W4-02/W4-09/W4-10；breaking command migration。 |
