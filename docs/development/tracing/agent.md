...
| `ERR_AGENT_FIX_BRIDGE_UNAVAILABLE` | `LBR-AGENT-010` | `review --fix` 无法发现或授权活跃的 `libra code --control write` runtime；`investigate fix` 尚未接线 | DF-03 / AG-23 |
| `ERR_AGENT_UNTRUSTED_SEED_FOR_MUTATION` | `LBR-AGENT-011` | `ReviewFixInput::UntrustedSeed` 在 control discovery 之前拒绝，任何外部 finding/issue/transcript 都不能经 DF-03 admission 进入运行时 | DF-03 / DF-09 |
