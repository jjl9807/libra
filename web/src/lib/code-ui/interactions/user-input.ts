...
/**
 * Whether the browser UI may post interaction responses for this provider.
 *
 * W3-07 forwards managed Codex ask-mode approvals through the shared
 * `AgentRuntime` registry, so managed Codex is no longer blocked here.
 * The helper remains so callers keep a single gate if a future provider
 * needs a fail-closed respond path.
 */
export function browserInteractionRespondSupported(_provider: {
  provider: string;
  managed: boolean;
}): boolean {
  void _provider;
