...
export function SessionLifecycle({ api: injectedApi }: SessionLifecycleProps) {
  ...
  // Cancel uses withLease; do not gate the button on projected canWrite — a
  // freshly opened browser has canWrite=false until the first write attaches.
  const canCancel = Boolean(snapshot);
